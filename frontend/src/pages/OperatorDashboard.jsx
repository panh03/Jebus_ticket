...
const OperatorDashboard = () => {
  ...
  const fetchAllData = async () => {
     try {
       const res = await axios.get("http://localhost:5000/api/operator/trips");
       setTrips(res.data);
    ...
    try {
      if (activeTab === "trips") {
        const url = selectedRoute 
           ? `http://localhost:5000/api/operator/trips?routeId=${selectedRoute.id}`
           : "http://localhost:5000/api/operator/trips";
        const res = await axios.get(url);
        setTrips(res.data);
      } else if (activeTab === "routes") {
        const res = await axios.get("http://localhost:5000/api/operator/routes");
        setRoutes(res.data);
      } else if (activeTab === "promotions") {
        const res = await axios.get("http://localhost:5000/api/operator/promotions");
        setPromotions(res.data);
      } else if (activeTab === "requests") {
        const res = await axios.get("http://localhost:5000/api/operator/cancellations");
  ...
  const updateTripStatus = async (tripId, newStatus) => {
    try {
      await axios.put(`http://localhost:5000/api/operator/trips/${tripId}/status`, { status: newStatus });
      fetchActiveTabData();
    } catch (err) {
  ...
  const processCancellation = async (requestId, status) => {
     try {
        await axios.put(`http://localhost:5000/api/operator/cancellations/${requestId}`, { status });
  ...
  const viewTripDetails = async (instanceId) => {
    ...
    try {
       const [passRes, seatsRes] = await Promise.all([
          axios.get(`http://localhost:5000/api/operator/trips/${instanceId}/passengers`),
          axios.get(`http://localhost:5000/api/operator/trips/${instanceId}/seats`)
       ]);
